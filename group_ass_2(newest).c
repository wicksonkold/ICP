...
int main() {
    ...
    printf("7. Export / Dreate Remarks: \n");
...
void remarks(){
    ...
    printf("Each line allow to input 100 character:\n");
    top:
   	printf("Enter the above record number that you want to create remarks:\n");
   	gets(filename);
	...
	here:
		...
		fprintf(file,"%s\n",remarks);
	again:
		printf("Do you want to enter anthoer the remark for that record? Yes<Y>/No<N>");
		scanf("%c%c", &yesno, &ch);
		switch (yesno) {
	        case 'y':
	        case 'Y':
